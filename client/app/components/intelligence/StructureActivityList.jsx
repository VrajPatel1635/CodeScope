...
export default function StructureActivityList({ structures }) {
  if (!structures || structures.length === 0) return null;

  return (
    <div className="flex flex-col gap-2">
      <h3 className="text-[11px] font-bold uppercase tracking-wider" style={{ color: "var(--text-muted)" }}>Active Structures</h3>
      <div className="flex flex-col gap-1.5 mt-1">
        {structures.map((s, idx) => (
          <div key={idx} className="flex justify-between items-center py-0.5 border-b border-opacity-10 last:border-0" style={{ borderColor: "var(--border-color)" }}>
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs font-mono" style={{ color: "var(--text-secondary)" }}>{s.percentage}%</span>
            </div>
          </div>
        ))}
      </div>
